...
const { visibilityMap, updateGuides, getSnapDelta, movingAwayX, movingAwayY } = useSnapping(
	selectionBoxRef,
	slideRef,
)
...
const getTotalPositionDelta = (delta) => {
	...
	if (movingAwayX.value && Math.abs(delta.x) < 2) {
		delta.x = 0
		snapDelta.x = 0
	}

	if (movingAwayY.value && Math.abs(delta.y) < 2) {
		delta.y = 0
		snapDelta.y = 0
	}
