...
import { slideIndex, loadSlide } from '@/stores/slide'
...
const transitionMap = {
	'Slide In': {
		...
		enter: {
			...
			transition: `transform ${state.value[slideIndex.value].transitionDuration}s ease-out`,
		},
		beforeLeave: {
			transition: 'none',
		},
	...
	Fade: {
		...
		enter: {
			transition: `opacity ${state.value[slideIndex.value].transitionDuration}s`,
		},
		beforeLeave: {},
		leave: {
			transition: `opacity ${state.value[slideIndex.value].transitionDuration}s`,
...
const applyTransitionStyles = (hook) => {
	const styles = transitionMap[state.value[slideIndex.value].transition][hook]
...
const beforeSlideEnter = (el) => {
	if (!state.value[slideIndex.value].transition) return
	applyTransitionStyles('beforeEnter')
}

const slideEnter = (el, done) => {
	if (!state.value[slideIndex.value].transition) return done()
	el.offsetWidth
